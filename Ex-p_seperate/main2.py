...
from CondaPy.CondaPy import dec
...
def press(button):
    if button == "EXIT":
        app.stop()
    if button == "Submit":
        app.setLabel("x",str(dec(app.getEntry("Encrypt"),199)))
        copy(app.getLabel("x"))
def b_exit():
    app.setSticky("ne")
    app.setExpand("both")
...
def b_title():
    ...
    app.addLabel("x","This is where encrypted text goes")
def b_Encrypt():
    app.addLabelEntry("Decrypt")
